...
import static me.antcode.datacollection.CSVUtils.correctNames;
...
public class Matchup {
    ...
    public Player findPlayerObject(String playerName) {
        // Check if playerName needs correction
        if (correctNames.containsKey(playerName)) {
            for (Player player : totalPlayers){
                if (player.getName().equalsIgnoreCase(correctNames.get(playerName))) return player;
                // Get the corrected name
            }
        }

        // Iterate through the list of players to find the matching player
        for (Player player : totalPlayers) {
